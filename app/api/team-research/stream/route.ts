...
export async function POST(req: NextRequest) {
  ...
  const isLawQuestion = hasSearchKeys && /VAT|vat|ภาษี|มาตรา|พ\.ร\.บ\.|พ\.ร\.ก\.|กฎหมาย|ประมวลรัษฎากร|สรรพากร|หัก ณ ที่จ่าย|ยกเว้น|อากร|ภาษีซ้อน/.test(question);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (mode === "qa") {
        ...
        let searchContext = "";
        if ((agent.useWebSearch || isLawQuestion) && (serperKey || serpApiKeyVal)) {
      ...
      if (chairApiKey && allContext.length > 0) {
        try {
          ...
          const result = await callLLM(chairman.provider, chairman.model, chairApiKey, chairman.baseUrl, [
            {
              role: "system",
              content: `${companyContext}คุณเป็นประธานการประชุมในบทบาท ${chairman.role} มีหน้าที่สรุปมติที่ประชุมให้ชัดเจน ถูกต้อง ครบถ้วน ห้ามสรุปผิดจากข้อเท็จจริงที่นำเสนอ${mode === "close" && allRounds && allRounds.length > 1 ? ` (การประชุมนี้มี ${allRounds.length} วาระ สรุปรวมทั้งหมด)` : ""}${failureNote}`,
            },
            {
              role: "user",
