...
export async function POST(req: NextRequest) {
  ...
  // Auto web search for law/tax questions (even if agent hasn't enabled web search)
  const hasSearchKeys = !!(serperKey || serpApiKeyVal);
  const isLawQuestion = /VAT|vat|ภาษี|มาตรา|พ\.ร\.บ\.|พ\.ร\.ก\.|กฎหมาย|ประมวลรัษฎากร|สรรพากร|หัก ณ ที่จ่าย|ยกเว้น|อากร|ภาษีซ้อน|ขนส่ง|เงินได้|ลดหย่อน|นิติบุคคล|บุคคลธรรมดา|ดอกเบี้ย|เงินปันผล|ค่าเช่า|รับเหมา|วิชาชีพ|กำไรสุทธิ|รายจ่ายต้องห้าม|จดทะเบียน|ธุรกิจเฉพาะ|แสตมป์|อสังหาริมทรัพย์|ประกันชีวิต|กองทุนสำรอง|ประกันสังคม|บริจาค|คริปโท|WHT|PIT|CIT|SBT/.test(question);
  const doAutoSearch = isLawQuestion && hasSearchKeys;

  // Comprehensive Thai tax law knowledge from ประมวลรัษฎากร (rd.go.th) — embedded because LLMs often hallucinate wrong sections
  const THAI_TAX_KNOWLEDGE = isLawQuestion ? `\n\n📚 ข้อมูลอ้างอิง: ประมวลรัษฎากร (กรมสรรพากร rd.go.th) — ภาษี 4 ประเภทหลัก
      ...
      for (const agent of orderedAgents) {
        ...
        try {
          ...
          const result = await callLLM(agent.provider, agent.model, apiKey, agent.baseUrl, [
            {
              role: "system",
              content: `${companyContext}${agent.soul}${knowledgeContext}${THAI_TAX_KNOWLEDGE}${dataSourceContext}${historyContext}${fileContext}${mcpContext}${searchContext}${previousFindingsContext}`,
            },
            {
              role: "user",
      ...
      if (chairApiKey && allContext.length > 0) {
        try {
          ...
          const result = await callLLM(chairman.provider, chairman.model, chairApiKey, chairman.baseUrl, [
            {
              role: "system",
              content: `${companyContext}คุณเป็นประธานการประชุมในบทบาท ${chairman.role} มีหน้าที่สรุปมติที่ประชุมให้ชัดเจน ถูกต้อง ครบถ้วน ห้ามสรุปผิดจากข้อเท็จจริงที่นำเสนอ${mode === "close" && allRounds && allRounds.length > 1 ? ` (การประชุมนี้มี ${allRounds.length} วาระ สรุปรวมทั้งหมด)` : ""}${failureNote}${THAI_TAX_KNOWLEDGE}`,
            },
            {
              role: "user",
